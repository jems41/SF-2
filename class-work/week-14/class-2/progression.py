class Progression:
    ...
    '''
    def __init__(self, start=0): # initialize any required attributes
        self._current = start  # this _ means that this should only be used in this class

    def _advance(self): # private method, 
    ...
    def printProgression(self, n):
        ...
        print(' '.join(str(next(self)) for _ in range(n)))  # take the next values in this
# iterator and change it into a string

    def lstProgression(self, n):
        '''Return the next n values of the progression as a list.'''
        return [next(self) for _ in range(n)]  # returns a list of the next n progression 
# values
